...
// Initialize the form with matriceria data
const form = useForm({
    titulo: props.matriceria.titulo,
    descripcion: props.matriceria.descripcion,
    path: null // This will hold the file object
});

// Set initial image preview
onMounted(() => {
    imagePreview.value = `/storage/${props.matriceria.path}`;
});
...
const submit = () => {
    ...
    form.post(route('matriceria.update', props.matriceria.id), {
        data: formData,
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
                            ...
                            <div class="flex flex-col justify-between w-full md:w-2/3 text-black">
                                ...
                                <div class="mb-6.5 mt-4">
                                    <button type="submit" class="btn-primary w-full flex items-center justify-center"
                                        ...
                                        {{ form.processing ? 'Actualizando...' : 'Actualizar matriceria propia' }}
                                    </button>
                                </div>
                            </div>
                        </div>
